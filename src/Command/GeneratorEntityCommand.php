...
class GeneratorEntityCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): void
    {
        ...
        foreach ($entitiesRawName as $entityRawName)
        {
            ...
            if (count($entitySplit) == 1 && isset($entitySplit[0]) && $entitySplit[0] == $entityRawName)
            {
                $entitySplit = ['App', $entityRawName];
            }
            else if (empty($entitySplit) || in_array($entityRawName, $entitySplit) || count($entitySplit) != 2)
            {
